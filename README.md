...
目前版本新增「播放器」分頁，支援從裝置選取影片或音訊，以 Media3 播放並使用原生播放控制器。播放器可載入外部 SRT／VTT 字幕，並在播放畫面顯示字幕軌。

按下「開始離線即時聽譯」後，App 會重用既有的 Whisper／Vosk 離線語音模型進行辨識，並使用裝置端翻譯將結果轉為繁體中文；完成的字幕會按照播放位置同步顯示原文與翻譯。長時間媒體會在背景工作服務中處理，完成後仍可返回播放器查看。
